...
def donate():
    data = request.json
    try:
        donation = Donation(name=data['name'], amount=data['amount'],
...
def register():
    data = request.json
    required_fields = ['first_name', 'last_name', 'email', 'contact', 'primary_address', 'city', 'state', 'zipcode']
...
def get_registrations():
    ...
    return jsonify([{
        ...
        'email': r.email, 'contact': r.contact
...
def add_to_cart():
    data = request.json
    try:
        cart_item = CartItem(user_id=data['user_id'], product_id=data['product_id'],
...
def artist_register():
    ...
    try:
        ...
        db.session.commit()
        return jsonify({"message": "Artist registered successfully", "id": artist.id}), 201
    except Exception as e:
        db.session.rollback()
        return jsonify({"error": str(e)}), 400
